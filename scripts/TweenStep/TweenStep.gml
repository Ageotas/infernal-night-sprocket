...
{        
    ...
    if (_t[TWEEN.STATE] != TWEEN_STATE.STOPPED && _timeScale != 0)
    {
        ...
        if (_time > 0 && _time < _duration)
        {
            ...
            if (_property != TGMS_NULL__) script_execute(_property, script_execute(_t[TWEEN.EASE], _time, _t[TWEEN.START], _t[TWEEN.CHANGE], _duration), _t[TWEEN.DATA], _t[TWEEN.TARGET]);    
        }
        ...
        if (_t[TWEEN.TIME_SCALE] != 0) // Make sure time scale isn't "paused"
        {
            ...
            case TWEEN_MODE_ONCE:
                ...
                // Update property
                if (_property != TGMS_NULL__) script_execute(_property, _t[TWEEN.START] + _t[TWEEN.CHANGE]*(_time > 0), _t[TWEEN.DATA], _t[TWEEN.TARGET]);
            ...
            case TWEEN_MODE_BOUNCE:
                ...
                else
                {
                    ...
                    // Update property
                    if (_property != TGMS_NULL__) script_execute(_property, _t[TWEEN.START], _t[TWEEN.DATA], _t[TWEEN.TARGET]);
